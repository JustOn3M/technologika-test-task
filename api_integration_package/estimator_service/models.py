"""
Pydantic models for Estimator Service API.
Includes all models from Takeoff Service plus webhook-specific models.
"""
...
# ========================================
# CORE MODELS (from Takeoff Service)
# ========================================
...
class DeleteConditionAction(BaseModel):
    """Identifier for condition deletion."""
    id: UUID = Field(..., description="ID of condition to delete")


class DeleteTakeoffItemAction(BaseModel):
    """Identifier for takeoff item deletion."""
    id: UUID = Field(..., description="ID of takeoff item to delete")


class DeleteTakeoffZoneAction(BaseModel):
    """Identifier for takeoff zone deletion."""
    id: UUID = Field(..., description="ID of takeoff zone to delete")


class ConditionsAction(BaseModel):
    """Represents a single create/update/delete action in the change notification."""
    orderNumber: int = Field(..., description="Sequence order for processing multiple actions")
    actionName: Optional[str] = Field(None, description="Type of action - Create, Update, Delete")
    entityType: Optional[str] = Field(None, description="Type of entity affected - TakeoffZone, TakeoffItem, Condition")

    # Create/Update actions (one of these will be populated)
    condition: Optional[ConditionDto] = Field(None, description="Condition data for Create/Update actions")
    takeoffZone: Optional[TakeoffZoneDto] = Field(None, description="TakeoffZone data for Create/Update actions")
    takeoffItem: Optional[TakeoffItemDto] = Field(None, description="TakeoffItem data for Create/Update actions")

    # Delete actions (one of these will be populated)
    deleteCondition: Optional[DeleteConditionAction] = Field(None, description="Condition identifier for Delete actions")
    deleteTakeoffItem: Optional[DeleteTakeoffItemAction] = Field(None, description="TakeoffItem identifier for Delete actions")
    deleteTakeoffZone: Optional[DeleteTakeoffZoneAction] = Field(None, description="TakeoffZone identifier for Delete actions")


class ConditionsChange(BaseModel):
    """Webhook payload containing changes made in Takeoff."""
    documentId: UUID = Field(..., description="Document where changes occurred")
    pageNumber: int = Field(..., ge=1, description="Page number where changes occurred")
    actions: Optional[List[ConditionsAction]] = Field(None, description="List of actions performed (ordered by orderNumber)")
